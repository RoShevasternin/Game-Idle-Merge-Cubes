...
import com.lewydo.idlemergecubes.game.utils.GlobalEvents
import com.lewydo.idlemergecubes.game.utils.GlobalStageTargets
    ...
    private fun spawnFlyingLabels(target: ACube, coins: Long, xp: Long, color: Color) {
        ...
        screen.stageUI.root.addActor(
            AFlyingLabel(
                ...
                text   = "+$coins",
                style  = style,
                to     = GlobalStageTargets.getTarget(GlobalStageTargets.TargetType.COIN),
                side   = AFlyingLabel.Side.Left,
                onEnd  = { GlobalEvents.invokeEvent(GlobalEvents.EventType.END_FLY_COIN) }
        ...
        screen.stageUI.addActor(
            AFlyingLabel(
                ...
                text   = "+$xp XP",
                style  = style,
                to     = GlobalStageTargets.getTarget(GlobalStageTargets.TargetType.XP),
                side   = AFlyingLabel.Side.Right,
                onEnd  = { GlobalEvents.invokeEvent(GlobalEvents.EventType.END_FLY_XP) }
            ).also { it.setBounds(from, size) }
        )
    }
