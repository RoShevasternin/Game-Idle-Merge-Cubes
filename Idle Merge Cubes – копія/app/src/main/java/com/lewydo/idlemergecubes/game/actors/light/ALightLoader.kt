...
class ALightLoader(override val screen: LoaderScreen): AdvancedGroup() {
    ...
    private fun animateLights() {
        listLightImg.forEachIndexed { index, image ->
            ...
            val scaleWave = Actions.forever(
                Actions.sequence(
                    Actions.delay(ratio * 0.12f), // фазовий зсув
                    Actions.scaleTo(scaleMax, scaleMax, baseTime * 0.45f, Interpolation.pow2Out),
                    Actions.scaleTo(1f, 1f,       baseTime * 0.55f, Interpolation.sine)
                )
            )
            ...
            val rotation = if (isTop) {
                ...
                Actions.forever(
                    Actions.sequence(
                        Actions.rotateTo(5f * dir, baseTime * 0.65f, Interpolation.sine),
                        Actions.rotateTo(-5f * dir, baseTime * 0.65f, Interpolation.sine)
                    )
                )
            } else null
            ...
            val drift = if (index <= 2) {
                ...
                Actions.forever(
                    Actions.sequence(
                        Actions.moveBy(dist, dist, baseTime, Interpolation.sine),
                        Actions.moveBy(-dist, -dist, baseTime, Interpolation.sine)
                    )
                )
            } else null
        ...
        listLightImg.forEachIndexed { index, image ->
            ...
            image.addAction(
                Actions.sequence(
                    ...
                    Actions.parallel(
                        Actions.scaleTo(0.7f, 0.7f, 0.12f, Interpolation.pow2In),
                    ...
                    Actions.parallel(
                        ...
                        Actions.alpha(0f, 0.5f, Interpolation.pow3Out),

                        // Додаємо фінальний оберт для динаміки
                        Actions.rotateBy(if (index % 2 == 0) 120f else -120f, 0.6f, Interpolation.exp5Out)
                    ),
