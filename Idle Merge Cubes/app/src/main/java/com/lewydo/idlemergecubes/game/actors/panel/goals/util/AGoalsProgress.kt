...
class AGoalsProgress(override val screen: AdvancedScreen) : AdvancedGroup() {

    private val LENGTH      = 1570f
    private val onePercentX = LENGTH / 100f

    private val backgroundImage = Image(gdxGame.assetsAll.goals_progress_bg)
    private val progressImage   = Image(gdxGame.assetsAll.goals_progress)
    private val mask            = AMask(screen, gdxGame.assetsAll.MASK_GOALS_PROGRESS)
    ...
    private fun AdvancedGroup.addMask() {
        addActor(mask)
        mask.setBounds(2f, 2f, LENGTH, 30f)
