...
import com.lewydo.idlemergecubes.game.utils.font.msdf.effects.MsdfEffect
import com.lewydo.idlemergecubes.game.utils.font.msdf.effects.MsdfEffectContext
...
class MsdfLabel(
    ...
    constructor(text: CharSequence, style: MsdfStyle, worldSize: Float = style.size) : this(
        style.manager, style.font, text, worldSize, style.color,
    ) {
    ...
    fun setLetterSpacing(percent: Float): MsdfLabel { letterSpacingPercent = percent; return this }
    fun setLineHeight(percent: Float): MsdfLabel { lineHeightPercent = percent; return this }
    fun figmaBox(enabled: Boolean = true): MsdfLabel { useFigmaBox = enabled; return this }

    fun addEffect(effect: MsdfEffect): MsdfLabel {
        effects.add(effect); effects.sortBy { it.layer }; return this
    }
    fun clearEffects(): MsdfLabel { effects.clear(); return this }
    ...
    override fun getPrefWidth(): Float {
        ...
        pushSpacing()
        var w = super.getPrefWidth()
        popSpacing()
        if (letterSpacingPercent != 0f) {
            w -= (font.glyphSize * letterSpacingPercent / 100f).toInt() * fontScaleX
        }
        return w
    }
